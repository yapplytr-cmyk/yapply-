  ...
  const loadAdminListingCollections = async () => {
    if (!adminListingCollectionsPromise) {
      adminListingCollectionsPromise = (async () => {
        ...
        // Admin: only show seed items if backend is completely unreachable
        const backendReachable = clientListingsResult.status === "fulfilled" || professionalListingsResult.status === "fulfilled";
        const mergedClientSeedItems = mergeAdminListingSeeds(remoteClientListings, backendReachable ? [] : clientSeedItems);
        const mergedProfessionalSeedItems = mergeAdminListingSeeds(remoteProfessionalListings, backendReachable ? [] : professionalSeedItems);
...
async function loadMarketplaceRuntimeData(page, listingType, listingId) {
  ...
  if (page === "admin-dashboard") {
    ...
    // If at least one PG query succeeded, the backend is reachable — hide seed/demo items
    const adminBackendReachable = clientListingsResult.status === "fulfilled" || professionalListingsResult.status === "fulfilled";
    return {
