...
# ── 5. Inquiry received — notify developer ───────────────

def send_inquiry_received(listing: dict[str, Any], inquiry: dict[str, Any]) -> None:
  """Notify the developer (listing owner) that a client sent an inquiry."""
  # Find developer email from listing owner
  to = _get(listing, "ownerEmail") or _get(listing, "contact", "email")
  if not to:
    # Try to look up the owner profile
    owner_id = _get(listing, "ownerUserId")
    if owner_id:
      try:
        from .supabase import get_profile_by_id
        profile = get_profile_by_id(owner_id)
        if profile:
          to = profile.get("email") or ""
      except Exception:
        pass

  listing_title = _get(listing, "title") or _get(listing, "name") or "your listing"
  owner_name = _get(listing, "ownerName") or _get(listing, "contact", "fullName") or ""

  sender_name = inquiry.get("fullName") or "A potential client"
  sender_email = inquiry.get("email") or ""
  sender_message = inquiry.get("message") or ""

  greeting = f"Hi {owner_name}," if owner_name else "Hi,"

  rows = [("From", sender_name)]
  if sender_email:
    rows.append(("Email", sender_email))
  rows.append(("Listing", listing_title))

  message_html = ""
  if sender_message:
    short_msg = sender_message[:300] + ("\u2026" if len(sender_message) > 300 else "")
    message_html = _proposal_block(short_msg)

  body = (
    _heading("New inquiry on your listing")
    + _paragraph(greeting)
    + _paragraph(f'Someone has sent an inquiry about your listing {_accent(f"&ldquo;{listing_title}&rdquo;")} on Yapply.')
    + _detail_card(rows, "Inquiry Details")
    + message_html
    + _cta_button("View Your Dashboard", "https://yapplytr.com/developer-dashboard.html")
  )
