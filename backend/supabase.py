...
def _supabase_binary_request(
  ...
  *,
  ...
  request = Request(f"{SUPABASE_URL}{path}", method=method, data=payload, headers=headers)
...
def verify_signup_otp(email: str, token: str) -> dict[str, Any]:
  """Verify a signup OTP code via Supabase POST /auth/v1/verify."""
  _ensure_public_config()
  ...
  try:
    ...
    body = error.read()
    try:
      err_data = _decode_json(body)
      msg = err_data.get("msg") or err_data.get("error_description") or err_data.get("message") or "Invalid or expired verification code."
    except Exception:
      msg = "Invalid or expired verification code."
    raise SupabaseError("OTP_INVALID", msg, 400) from error
  except URLError as error:
    raise SupabaseError("AUTH_UNAVAILABLE", "Authentication is not available right now.", 503) from error

  if not isinstance(data, dict):
    raise SupabaseError("SUPABASE_INVALID_RESPONSE", "Supabase returned an invalid verification response.", 502)

  return data


def resend_signup_otp(email: str) -> None:
  """Resend a signup OTP email via Supabase POST /auth/v1/resend."""
  _ensure_public_config()
  ...
  try:
    ...
    try:
      err_data = _decode_json(body)
      msg = err_data.get("msg") or err_data.get("message") or "Could not resend the verification code."
    except Exception:
      msg = "Could not resend the verification code."
    raise SupabaseError("OTP_RESEND_FAILED", msg, 429) from error
  except URLError as error:
    raise SupabaseError("AUTH_UNAVAILABLE", "Authentication is not available right now.", 503) from error
...
def _normalize_auth_user_account(user: dict[str, Any]) -> dict[str, Any] | None:
  ...
  return {
    ...
    "workDescription": _read_auth_user_field(user, "work_description", "workDescription"),
    "createdAt": user.get("created_at"),
    "updatedAt": user.get("updated_at") or user.get("last_sign_in_at"),
  }


def _resolve_public_role_from_account(
  auth_account: dict[str, Any] | None,
  existing_profile: dict[str, Any] | None = None,
  expected_role: str = "",
) -> str:
  candidates = [
    auth_account.get("role") if auth_account else "",
    existing_profile.get("role") if existing_profile else "",
    expected_role,
  ]

  for candidate in candidates:
    normalized = _normalize_role(candidate)
    if normalized in {"client", "developer"}:
      return normalized

  has_developer_signals = any(
    [
      auth_account.get("companyName") if auth_account else None,
      auth_account.get("professionType") if auth_account else None,
      auth_account.get("serviceArea") if auth_account else None,
      auth_account.get("specialties") if auth_account else None,
      existing_profile.get("companyName") if existing_profile else None,
      existing_profile.get("professionType") if existing_profile else None,
      existing_profile.get("serviceArea") if existing_profile else None,
      existing_profile.get("specialties") if existing_profile else None,
    ]
  )
  if has_developer_signals:
    return "developer"

  has_client_signals = any(
    [
      auth_account.get("preferredRegion") if auth_account else None,
      existing_profile.get("preferredRegion") if existing_profile else None,
    ]
  )
...
def _repair_public_profile_from_auth_user(
  user: dict[str, Any],
  existing_profile: dict[str, Any] | None = None,
  expected_role: str = "",
) -> dict[str, Any] | None:
  auth_account = _normalize_auth_user_account(user)
  resolved_role = _resolve_public_role_from_account(auth_account, existing_profile, expected_role)

  if not auth_account or not resolved_role:
    return existing_profile

  if existing_profile and _normalize_role(existing_profile.get("role")) == resolved_role:
    return existing_profile

  fallback_profile = existing_profile or {}
  email = auth_account.get("email") or fallback_profile.get("email") or ""

  return upsert_profile(
    user_id=auth_account["id"],
    email=email,
    role=resolved_role,
    full_name=auth_account.get("fullName") or fallback_profile.get("fullName") or email or "Yapply User",
    phone_number=auth_account.get("phoneNumber") or fallback_profile.get("phoneNumber"),
    company_name=auth_account.get("companyName") or fallback_profile.get("companyName"),
    profession_type=auth_account.get("professionType") or fallback_profile.get("professionType"),
    service_area=auth_account.get("serviceArea") or fallback_profile.get("serviceArea"),
    years_experience=auth_account.get("yearsExperience") or fallback_profile.get("yearsExperience"),
    specialties=auth_account.get("specialties") or fallback_profile.get("specialties"),
    preferred_region=auth_account.get("preferredRegion") or fallback_profile.get("preferredRegion"),
    website=auth_account.get("website") or fallback_profile.get("website"),
    avatar_url=fallback_profile.get("avatarUrl"),
    username=auth_account.get("username") or fallback_profile.get("username"),
    status=fallback_profile.get("status") or "active",
  )


def _verify_public_access_token(access_token: str) -> tuple[dict[str, Any], dict[str, Any]]:
  if not access_token:
  ...
  profile_role = _normalize_role(profile.get("role")) if profile else ""

  if not profile or profile_role not in {"client", "developer"}:
    profile = _repair_public_profile_from_auth_user(auth_user, profile, "")
...
def _decode_account_avatar_data_url(data_url: str) -> tuple[str, bytes]:
  match = re.match(r"^data:([^;,]+);base64,(.+)$", data_url, re.IGNORECASE | re.DOTALL)
  if not match:
    raise SupabaseError("PROFILE_PICTURE_INVALID", "The selected profile picture data is invalid.", 400)

  content_type = match.group(1).strip().lower()
  if content_type not in ALLOWED_ACCOUNT_AVATAR_MIME_TYPES:
    raise SupabaseError("PROFILE_PICTURE_INVALID", "The selected profile picture format is not supported.", 400)

  try:
    raw_bytes = base64.b64decode(match.group(2), validate=True)
  except ValueError as error:
    raise SupabaseError("PROFILE_PICTURE_INVALID", "The selected profile picture data is invalid.", 400) from error

  if not raw_bytes:
    raise SupabaseError("PROFILE_PICTURE_INVALID", "The selected profile picture could not be read.", 400)

  if len(raw_bytes) > MAX_ACCOUNT_AVATAR_BYTES:
    raise SupabaseError("PROFILE_PICTURE_TOO_LARGE", "The selected profile picture is too large.", 400)

  return content_type, raw_bytes


def _guess_account_avatar_extension(content_type: str, original_name: str = "") -> str:
  original_name = original_name.strip().lower()
  if "." in original_name:
    candidate = f".{original_name.rsplit('.', 1)[-1]}"
...
def _ensure_storage_bucket(bucket_id: str, *, public: bool = True) -> None:
  try:
    _supabase_request(
      "/storage/v1/bucket",
      method="POST",
...
def _upload_account_avatar(bucket_id: str, object_path: str, raw_bytes: bytes, content_type: str) -> None:
  _supabase_binary_request(
    f"/storage/v1/object/{bucket_id}/{quote(object_path, safe='/')}",
    method="POST",
    payload=raw_bytes,
    use_service_role=True,
...
def _build_public_storage_url(bucket_id: str, object_path: str) -> str:
  return f"{SUPABASE_URL}/storage/v1/object/public/{bucket_id}/{quote(object_path, safe='/')}"


def _update_auth_user_metadata(user_id: str, metadata: dict[str, Any]) -> dict[str, Any]:
  return _update_auth_user_record(user_id, metadata=metadata)


def _update_auth_user_record(
  user_id: str,
  *,
  email: str | None = None,
  ...
  user = response.get("user") if isinstance(response, dict) and isinstance(response.get("user"), dict) else response
  if not isinstance(user, dict) or not user.get("id"):
    raise SupabaseError("ACCOUNT_SETTINGS_UPDATE_FAILED", "The account settings could not be updated.", 502)
...
def require_public_access(access_token: str, expected_role: str = "") -> tuple[dict[str, Any], dict[str, Any]]:
  auth_user, profile = _verify_public_access_token(access_token)
  if expected_role and profile.get("role") != expected_role:
    raise SupabaseError("ROLE_MISMATCH", "This account does not match the required role.", 403)
  return auth_user, profile
...
def authenticate_public_user(email: str, password: str, expected_role: str = "") -> dict[str, Any]:
  ...
  profile = get_profile_by_id(user_id)
  profile_role = _normalize_role(profile.get("role")) if profile else ""
  if isinstance(user_payload, dict) and (not profile or profile_role not in {"client", "developer"}):
    profile = _repair_public_profile_from_auth_user(user_payload, profile, expected_role)
  if not profile:
  ...
  return {
    ...
    "user": {
      ...
      "workDescription": _read_auth_user_field(user_payload, "work_description", "workDescription"),
    },
  }


def update_own_account_settings(access_token: str, payload: dict[str, Any]) -> dict[str, Any]:
  auth_user, profile = _verify_public_access_token(access_token)
  username = str(payload.get("username") or "").strip()
  email = str(payload.get("email") or auth_user.get("email") or profile.get("email") or "").strip().lower()
  password = str(payload.get("password") or "")
  profile_picture_type = str(payload.get("profilePictureType") or "default").strip().lower()
  profile_picture_id = str(payload.get("profilePictureId") or "").strip()
  preserve_uploaded_picture = bool(payload.get("preserveProfilePictureUpload"))
  work_description = str(payload.get("workDescription") or "").strip()

  if not username:
    raise SupabaseError("USERNAME_REQUIRED", "Please enter a username.", 400)
  ...
  if profile_picture_type not in {"default", "upload"}:
    raise SupabaseError("PROFILE_PICTURE_INVALID", "The selected profile picture type is invalid.", 400)
  ...
  if profile_picture_type == "default":
    ...
    if upload_data_url:
      ...
      if not profile_picture_url:
        raise SupabaseError("PROFILE_PICTURE_INVALID", "The current profile picture could not be preserved.", 400)
    else:
      raise SupabaseError("PROFILE_PICTURE_INVALID", "The selected profile picture could not be saved.", 400)

  current_metadata = auth_user.get("user_metadata") if isinstance(auth_user.get("user_metadata"), dict) else {}
  next_metadata = {
    **current_metadata,
    "username": username,
    "profile_picture_type": profile_picture_type,
    "profile_picture_id": profile_picture_id or "",
    "profile_picture_url": profile_picture_url or "",
    "profile_picture_path": "",
  }

  if profile.get("role") == "developer":
    next_metadata["work_description"] = work_description

  _update_auth_user_record(
    str(profile["id"]),
    email=email,
  ...
  profile_query = urlencode({"id": f"eq.{profile['id']}", "select": PROFILE_COLUMNS})
  response = _supabase_request(
  ...
  rows = response if isinstance(response, list) else []
  updated_profile = normalize_profile_row(rows[0]) if rows else None
  ...
  return {
    "user": {
      ...
      "workDescription": next_metadata.get("work_description") or "",
    }
  }
...
def register_public_user(payload: dict[str, Any]) -> dict[str, Any]:
  ...
  # (via Resend SMTP). The user must verify the code before signing in.
  signup_result = sign_up_with_password(email, password, metadata)

  # Supabase returns a user object even before confirmation.
  user_id = ""
  ...
  # Check if Supabase auto-confirmed (email confirmation disabled in dashboard).
  session_data = signup_result if isinstance(signup_result, dict) else {}
  has_session = bool(session_data.get("access_token") and session_data.get("refresh_token"))
...
def complete_signup_verification(email: str, token: str, password: str) -> dict[str, Any]:
  ...
  if isinstance(verify_data, dict):
    user_obj = verify_data.get("user") if isinstance(verify_data.get("user"), dict) else verify_data
    user_id = str(user_obj.get("id") or "").strip()
  ...
  if not profile:
    raise SupabaseError("ACCOUNT_PROFILE_MISSING", "The account profile could not be loaded.", 500)

  if profile.get("status") != "active":
    raise SupabaseError("ACCOUNT_DISABLED", "This account has been disabled.", 403)
