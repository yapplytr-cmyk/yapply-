import { deleteBackendMarketplaceListing, deleteMarketplaceListing, getSubmittedListings } from "./marketplaceStore.js";
...
function buildMarketplaceItems(type, seedItems) {
  ...
  const baseItems = [...submittedItems, ...seedItems].filter((item, index, items) => {
    const itemKey = getListingKey(type, item);
    return items.findIndex((candidate) => getListingKey(type, candidate) === itemKey) === index;
  });
...
export async function deleteManagedListing(type, listing) {
  const store = readStore();
  const section = store.marketplace[type];

  if (listing.id) {
    await deleteBackendMarketplaceListing(listing.id);
  }
