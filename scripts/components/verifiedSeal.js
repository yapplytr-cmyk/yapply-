...
export function verifiedSealMarkup(locale) {
  ...
  return `<span class="yapply-verified-seal" title="${title}" aria-label="${title}" style="position:absolute;right:-2px;bottom:-2px;width:36%;min-width:15px;max-width:40px;line-height:0;pointer-events:none;filter:drop-shadow(0 2px 5px rgba(0,0,0,0.35))"><svg viewBox="0 0 48 48" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="ypVerifiedSeal" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#A8DCEE"/><stop offset="0.55" stop-color="#2A8DC8"/><stop offset="1" stop-color="#155B8A"/></linearGradient></defs><path d="${SEAL_PATH}" fill="url(#ypVerifiedSeal)" stroke="#ffffff" stroke-width="2" stroke-linejoin="round"/><path d="M15.5 24.4l5.4 5.4 11.8-12.6" fill="none" stroke="#ffffff" stroke-width="3.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span>`;
}
...
export function verifiedCheckBadge(locale) {
  ...
  return `<span class="yapply-verified-chip" title="${locale === "tr" ? "Doğrulanmış Üye" : "Verified Member"}" style="display:inline-flex;align-items:center;gap:3px;background:linear-gradient(135deg,#2A8DC8,#7EC8E3);color:#FFFFFF;font-size:0.6rem;font-weight:800;letter-spacing:0.02em;padding:2px 7px;border-radius:999px;vertical-align:middle;margin-left:6px;box-shadow:0 1px 5px rgba(27,111,168,0.4)"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.6" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 12.5l5 5 10-11"/></svg>${label}</span>`;
}
