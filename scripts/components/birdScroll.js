/**
 * Bird Scroll Animation — Isolated overlay component (v3).
 *
 * Renders the bird MP4 onto a canvas with real-time chroma-key removal
 * of the black background, producing a clean transparent bird graphic.
 * Playback is scrubbed by scroll position — not autoplay.
 *
 * v3: Mobile-specific refinements for visibility, timing, size, and
 * smoothness. Desktop behavior is unchanged.
 *
 * This file adds a single fixed-position element to the DOM and
 * does NOT modify any existing component, layout, or style.
 */

const VIDEO_SRC = "./assets/bird-scroll.mp4";

/* Chroma-key tuning.
   A pixel is considered "black background" only if BOTH:
   1. luminance is below KEY_THRESHOLD, AND
   2. color variance (max - min of R,G,B) is below SAT_THRESHOLD
   This preserves dark bird features (eyes, belt) that have
   slight color tints, while still removing the flat black bg. */
const KEY_THRESHOLD = 10;   // very tight: only near-pure-black pixels
const KEY_SOFT = 6;         // minimal soft edge
const SAT_THRESHOLD = 8;    // max channel spread to count as "flat black"
const SAT_SOFT = 4;         // soft-edge zone for saturation
...
function getSectionBounds() {
  ...
  /* Desktop — animation starts early, completes at How It Works,
     reverse begins only after that section. */
  return {
...
export function setupBirdScroll() {
  ...
  /* Frame cache — bigger on mobile for smoother scrubbing */
  ...
  /* ── Visible display canvas (fixed overlay) ─────────────── */
  const wrap = document.createElement("div");
  wrap.className = "bird-scroll-overlay";

  if (mobile) {
    /* MOBILE: bigger bird, properly on-screen, right edge */
    ...
    /* DESKTOP: bird in its correct visual location */
    Object.assign(wrap.style, {
      position: "fixed",
      right: "-10px",
      top: "25%",
  ...
  let vh = 0;
  let cropH = 0;
  let renderW = 0; // actual canvas render width (may be downscaled on mobile)
  let renderH = 0;
  let duration = 0;
  let ready = false;
  ...
  /* ── Chroma-key: remove black background, preserve dark features ── */
  /* Same thresholds for BOTH themes — the per-pixel keying must
     preserve dark bird features (eyes, belt) identically.
     Light-mode outline is handled via CSS drop-shadow on the wrapper. */
  ...
  /* Light mode: hide the bird entirely — the chroma-key fringe
     and dark outline are too visible against light backgrounds.
     Dark mode: bird displays normally, no filter needed. */
  function applyThemeVisibility() {
    if (isLightMode()) {
      ...
      /* opacity will be set by the scroll handler */
    }
  }
  ...
  /* On mobile, use a smoothed/interpolated scroll value to
     reduce choppiness from touch-scroll momentum. */
  let smoothY = window.scrollY;
  const SMOOTH_FACTOR = mobile ? 0.25 : 1; // 1 = no smoothing (desktop)
  ...
  function onScroll() {
    ...
    requestAnimationFrame(() => {
      ...
      if (mobile) {
        /* Lerp toward actual scroll position for softer motion */
        smoothY += (window.scrollY - smoothY) * SMOOTH_FACTOR;
        /* Keep running if we haven't converged */
  ...
  function update() {
    /* Bird is hidden in light mode — skip all work */
  ...
  /* Mobile Safari ignores preload="auto" and does not decode video
     frame data until the video has actually played.  This means
     video.currentTime seeking produces empty frames on canvas.
     Fix: briefly play() then pause() to prime the decoder, then
     seek to frame 0 so the bird is ready for scroll-scrubbing. */
  let primed = false;
